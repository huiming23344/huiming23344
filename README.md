## Hi there 👋

<!--
**huiming23344/huiming23344** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->

Hi 👋 I'm Huiming, a backend and cloud infrastructure engineer at Baidu.

- 🎓 B.E. in Software Engineering, Wuhan University (2025)
- 🔭 Working on **cloud infrastructure systems** built with **Golang and Python**
- ☁️ Focused on **OpenStack-based IaaS platforms** and **distributed task orchestration**
- ⚙️ Interested in **VM lifecycle management**, **system reliability**, and **consistency guarantees**
- 🌱 Currently learning **LangGraph** for building **AI agent systems** and **LLM-based workflow orchestration**
- 💬 Languages: Chinese / English
- 📫 Reach me at: lgm05251@outlook.com

## 📈 my stats
![Anurag's GitHub stats](https://github-readme-stats.vercel.app/api?username=huiming23344&show_icons=true&count_private=true&hide=issues) [![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=huiming23344&layout=compact)](https://github.com/anuraghazra/github-readme-stats)
